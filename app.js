/* ==========================================================================
   ...
   无后端依赖；核心渲染不依赖网络与存储。支持多语言（见 i18n.js）。
   ========================================================================== */
(function () {
  "use strict";
  ...
  var I18N = window.STARSCOPE_I18N || {};
  var LANGS = window.STARSCOPE_LANGS || [{ code: "zh-CN", label: "简体中文" }];
  var LANG_KEY = "starscope_lang";
  ...
  function catName(cat) { return t("cat_" + cat.id + "_name"); }
  function catDesc(cat) { return t("cat_" + cat.id + "_desc"); }

  function detectLang() {
    var saved = store ? (function () { try { return store.getItem(LANG_KEY); } catch (e) { return null; } })() : null;
    if (saved) {
      for (var i = 0; i < LANGS.length; i++) { if (LANGS[i].code === saved) return saved; }
    }
    var nav = (navigator.language || navigator.userLanguage || "zh-CN");
    var lower = nav.toLowerCase();
    for (var j = 0; j < LANGS.length; j++) {
      if (LANGS[j].code.toLowerCase() === lower) return LANGS[j].code;
    }
    if (lower.indexOf("zh") === 0) {
      return (lower.indexOf("tw") !== -1 || lower.indexOf("hk") !== -1 ||
        lower.indexOf("hant") !== -1) ? "zh-TW" : "zh-CN";
    }
    for (var k = 0; k < LANGS.length; k++) {
      if (lower.indexOf(LANGS[k].code.toLowerCase()) === 0) return LANGS[k].code;
    }
    return "zh-CN";
  }
  ...
  function setLang(code, opts) {
    opts = opts || {};
    var ok = false;
    for (var i = 0; i < LANGS.length; i++) { if (LANGS[i].code === code) ok = true; }
    if (!ok) code = "zh-CN";
    currentLang = code;
    document.documentElement.setAttribute("lang", code);
    if (store) { try { store.setItem(LANG_KEY, code); } catch (e) {} }
    applyStaticI18n();
    var sel = byId("lang-select");
    if (sel) sel.value = code;
    if (current && byId("view-result") && !byId("view-result").hidden) {
      renderResult(current, { noScroll: true });
    }
    if (!byId("view-error") || byId("view-error").hidden) { /* nothing */ }
    if (opts.silent !== true) toast(t("langSwitched") + " · " + langLabel(code));
  }
  ...
  function showView(id, noScroll) {
    VIEWS.forEach(function (v) {
      var el = byId(v);
      if (el) el.hidden = (v !== id);
    });
    if (!noScroll) window.scrollTo({ top: 0, behavior: "auto" });
  }
  ...
  function soulFor(stats) {
    var id = stats.topCat && stats.topCat.id ? stats.topCat.id : "other";
    return { id: id, title: t("soul_" + id + "_title"), desc: t("soul_" + id + "_desc") };
  }
  ...
  var StarMap = (function () {
    ...
    function showTip(s, evt) {
      ...
      tipEl.innerHTML =
        ...
        '<div class="tip-foot">' +
        '<span class="tip-cat"><i style="background:' + s.cat.color + '"></i>' + esc(catName(s.cat)) + "</span>" +
  ...
  function initBgStars() {
    ...
    function paint(now) {
      ...
      var t2 = now / 1000;
      stars.forEach(function (s) {
        var a = reduced ? s.a : s.a * (0.6 + 0.4 * Math.sin(t2 * s.sp + s.ph));
        ctx.globalAlpha = a;
  ...
  function renderResult(stats, opts) {
    opts = opts || {};
    current = stats;
    activeFilter = null;
    var p = stats.profile;
    ...
    byId("result-bio").textContent = stats.demo
      ? t("demoBio")
      : (p.bio || t("bioFallback"));

    var metaBits = [];
    if (p.company) metaBits.push(t("metaCompany", { v: p.company }));
    if (p.location) metaBits.push(t("metaLocation", { v: p.location }));
    if (p.public_repos !== undefined) metaBits.push(t("metaRepos", { v: fmt(p.public_repos) }));
    if (p.followers !== undefined) metaBits.push(t("metaFollowers", { v: fmt(p.followers) }));
    ...
    byId("starmap-title-user").textContent = t("titleUser", { login: p.login || "", n: stats.count });
    ...
    legend.innerHTML = stats.catStats.map(function (cs) {
      return '<span class="legend-item" data-cat="' + cs.cat.id + '" role="button" tabindex="0" ' +
        'aria-label="' + esc(t("legendAria", { cat: catName(cs.cat) })) + '"><i style="background:' + cs.cat.color + '"></i>' +
        esc(catName(cs.cat)) + '<span class="legend-count">' + cs.repos.length + "</span></span>";
    }).join("");

    // 星图
    byId("starmap-foot-note").textContent = stats.truncated
      ? t("footTruncated", { n: stats.count })
      : t("footAll", { n: stats.count });
    StarMap.init(stats);

    // 统计卡片
    var soul = soulFor(stats);
    byId("stat-count").textContent = fmt(stats.count);
    byId("stat-count-note").textContent = t("countNote", {
      cats: stats.categoryCount, langs: stats.langStats.length
    });
    byId("stat-stars").textContent = fmt(stats.totalStars);
    byId("stat-stars-note").textContent = stats.mostStarred
      ? t("starsTop", { name: stats.mostStarred.full_name })
      : t("noData");
    byId("stat-focus").textContent = focusIndex(stats);
    byId("stat-focus-note").textContent = t("focusNote", { cat: catName(stats.topCat) });
    ...
    if (stats.langStats.length > 7) {
      langHtml += '<div class="lang-more">' + t("langMore", { n: stats.langStats.length - 7 }) + "</div>";
    }
    byId("lang-list").innerHTML = langHtml || '<p class="empty-note">' + esc(t("emptyLang")) + "</p>";

    // 星座统计
    byId("cat-list").innerHTML = (stats.catStats.map(function (cs) {
      return '<div class="cat-row"><i style="background:' + cs.cat.color + '"></i>' +
        '<span class="cat-name">' + esc(catName(cs.cat)) +
        "<small>" + esc(catDesc(cs.cat)) + "</small></span>" +
        '<span class="cat-count">' + t("catCount", { n: cs.repos.length }) + "</span></div>";
    }).join("")) || '<p class="empty-note">' + esc(t("emptyCat")) + "</p>";

    // 意外宝藏
    if (stats.gem) {
      var g = stats.gem;
      byId("gem-name").textContent = g.full_name;
      byId("gem-desc").textContent = g.description || t("gemDescFallback");
      byId("gem-foot").innerHTML = "<span>★ " + fmt(g.stars) + "</span>" +
        "<span>" + esc(catName(g.cat)) + "</span>" +
        '<a href="' + esc(g.html_url) + '" target="_blank" rel="noopener">' + esc(t("viewRepo")) + "</a>";
    }

    // 技术灵魂
    byId("soul-name").textContent = soul.title;
    byId("soul-desc").textContent = soul.desc + t("soulTail", {
      cat: catName(stats.topCat), pct: Math.round(stats.topShare * 100)
    });
    ...
    showView("view-result", opts.noScroll);
  }
  ...
  function defaultFootNote() {
    ...
    return current.truncated
      ? t("footTruncated", { n: current.count })
      : t("footAll", { n: current.count });
  }
  ...
  function toggleLegendFilter(id) {
    ...
    if (activeFilter && current) {
      ...
      note.textContent = hit
        ? t("filterView", { cat: catName(hit.cat), n: hit.repos.length })
        : defaultFootNote();
    } else {
  ...
  function drawShareCard(stats, cb) {
    ...
    g.fillText(t("shareCardTitle"), 70, 92);
    ...
    // 数据行
    var metrics = [
      [t("shareMetricCount"), fmt(stats.count)],
      [t("shareMetricStars"), fmt(stats.totalStars)],
      [t("shareMetricCats"), String(stats.categoryCount)]
    ];
    ...
    g.fillText(t("shareFooter"), 70, 588);
  ...
  function loadingLines() {
    return [t("loading1"), t("loading2"), t("loading3"), t("loading4")];
  }
  ...
  function saveToken() {
    ...
    toast(val ? t("tokenSaved") : t("tokenCleared"));
  }
